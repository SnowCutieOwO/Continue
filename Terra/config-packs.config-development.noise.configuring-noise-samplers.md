...
![](images/rocky_noise.png)

在配置地形包时，有些参数会要求填入[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)。
...
关于 Terra 中完整的采样器类型和参数，请见“[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)”章节。
...
|2 倍频率|1 倍频率|0.5 倍频率|
|:---:|:---:|:---:|
|![](images/opensimplex2_64x64_freq_double.png)|![](images/opensimplex2_64x64.png)|![](images/opensimplex2_64x64_freq_half.png)|
...
可以通过 `frequency` 参数为[支持它的噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)设置频率：
...
这段配置中的噪声决定了[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)中材料的生成规律。为了更好理解它们之间的关系，如下的图片展示了使用这种调色板配置生成的世界：

![](images/palette_example.png)

## 其他噪声采样器方法

### 分形化

在某些情况下，[相干噪声](config-packs.config-development.noise.how-noise-samplers-work.md)太过平滑、重复，难以生成更加“真实”的地形。看看上述的图片，你可能注意到只有简单噪声的地形像*浸满雨滴的窗户*，这对我们添加更多细节不利。这时，我们就要用上**分形化**了。
...
|无分形|2倍频程|3倍频程|4倍频程|
|:---:|:---:|:---:|:---:|
|![](images/opensimplex2_1_octave.png)|![](images/opensimplex2_2_octaves.png)|![](images/opensimplex2_3_octaves.png)|![](images/opensimplex2_4_octaves.png)|
...
所有分形器和参数请参阅“[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)”章节。
...
|待扭曲的采样|参与扭曲的采样|
|:---:|:---:|
|![](images/checkerboard.png)|![](images/simplex_warp.png)|

经过域扭曲处理的结果如下所示：

![](images/checkerboard_warped.png)
