...
[创建](config-packs.config-development.config-files.md#创建配置文件)一个名为 `grass_palette.yml` 的空配置。
...
调色板由多个[调色板层](config-packs.config-documentation.config-objects.palettelayer.md)组成，而每个调色板层都包含对应的[方块](config-packs.config-documentation.config-objects.block.md)。

每[层调色板](config-packs.config-documentation.config-objects.palettelayer.md)也包括了方块在 Y 轴之间的分布层数。
...
通过[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)，调色板层可以从[方块](config-packs.config-documentation.config-objects.block.md)的[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)中选择，以此影响地形生成中的方块选择。
...
权重列表的相关讲述可以在[这里](config-packs.config-development.noise.how-noise-distributes-things.md)了解。
