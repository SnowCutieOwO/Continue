...
[创建一个空配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `grass_feature.yml`。

通过 `type` [参数](config-packs.config-development.the-config-system.md#参数)设置[配置类型](config-packs.config-development.the-config-system.md#配置类型)，并按如下所示填写 `id` 中的内容。
...
[分布器](config-packs.config-documentation.config-objects.distributor.md)决定了地物在世界中的 X 与 Z 轴放置位置。

将 `config-feature` 作为依赖引入，版本为 `1.+`。

这个附属提供了一系列可在地物配置文件内使用的[分布器](config-packs.config-documentation.config-objects.distributor.md)。
...
分布器类型的相关文档可以在[这里](config-packs.config-documentation.config-objects.distributor.md)找到。

`POSITIVE_WHITE_NOISE` 及其他噪声采样器的相关文档可以在[这里](config-packs.config-documentation.config-objects.noisesampler.md)找到。

:::

### 4. 添加地物定位器

[定位器](config-packs.config-documentation.config-objects.locator.md)决定了地物在世界上的 Y 轴位置。

将 `config-locators` 作为依赖引入，版本为 `1.+`。

这个附属提供了一系列可在地物配置文件内使用的[定位器](config-packs.config-documentation.config-objects.locator.md)。
...
`range` 有 `min`（最小值）与 `max`（最大值）两个[参数](config-packs.config-development.the-config-system.md#参数)。

::: info

其他几种定位器类型的相关文档可以在[这里](config-packs.config-documentation.config-objects.locator.md)找到。
...
通过使用 `AND` 定位器，我们可以使用多个[定位器](config-packs.config-documentation.config-objects.locator.md)，使得地物生成的条件更加精确。
...
`type` 为 `MATCH_SET` 的 `PATTERN` 定位器包含了两个[参数](config-packs.config-development.the-config-system.md#参数)，即 `blocks` 与 `offset`。
...
现在，我们可以利用之前添加的 `structure-block-shortcut` 作为，将单个方块当做[结构](config-packs.config-documentation.config-objects.structure.md)使用。
...
`structures` 父键包含了两个[参数](config-packs.config-development.the-config-system.md#参数)，即 `structures.structures` 和 `structures.distribution`。

`structures.structures` 决定了生成在世界上的结构或结构[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)。

`structure.distribution` 影响了结构选择结果的[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)。

::: tip

在如下带有[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)的示例中，地物可以从地物或方块的[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)中选取，以引导结构选择，如下所示。
...
权重列表的详细介绍可以在[这里](config-packs.config-documentation.config-objects.weightedlist.md)浏览。
