...
[群系提供器](config-packs.config-documentation.config-objects.biomeprovider.md)决定并配置生物群系在世界中的生成方式。

流水线式生物[群系提供器](config-packs.config-documentation.config-objects.biomeprovider.md)适合将生物群系平面分布。
...
`blend.sampler` 包含了[噪声取样器](config-packs.config-documentation.config-objects.noisesampler.md)及其[参数](config-packs.config-development.the-config-system.md#参数)，能够模糊群系之间的边缘区域。

::: info

`OPEN_SIMPLEX_2` 与其他噪声取样器的相关文档可以在[这里](config-packs.config-documentation.config-objects.noisesampler.md)找到。
...
`source.sampler` 通过[噪声采样器](config-packs.config-documentation.config-objects.noisesampler.md)分布初始群系。我们将其的值设置为 `CONSTANT`，因为它就是个简单的流水线源。

`source_biomes` 是一个[流水线群系](config-packs.config-documentation.config-objects.pipelinebiome.md)组成的[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)，作为初始样式。

在本示例中，我们会使用表示[流水线群系](config-packs.config-documentation.config-objects.pipelinebiome.md)的临时变量，之后会通过[流水线阶段](config-packs.config-documentation.config-objects.stage.md)替换为实际生物群系，否则地形包不会正常载入。
...
`REPLACE` 流水线阶段需要的[参数](config-packs.config-development.the-config-system.md#参数)为 `sampler`、`from` 和 `to`。

* `sampler` - 决定影响群系选择的[噪声取样器](config-packs.config-documentation.config-objects.noisesampler.md)
* `from` - 指定会被替换的[标签](config-packs.config-documentation.config-objects.tag.md)或群系。
* `to` - 指定替换 `from` 的[流水线群系](config-packs.config-documentation.config-objects.pipelinebiome.md)的[权重列表](config-packs.config-documentation.config-objects.weightedlist.md)。

权重列表的详细讲述可以在[这里](config-packs.config-development.noise.md)找到。
