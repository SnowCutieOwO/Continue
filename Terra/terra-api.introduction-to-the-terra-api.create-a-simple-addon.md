...
对于你的第一个附属，你可以直接复制粘贴这个验证文件，并按需要修改。在修改任何内容之前，请先在[这里](terra-api.api-concepts.manifest-addons.md)了解它们的作用。
...
让我们为附属添加一条启动时提醒。首先我们需要一个[记录器](https://www.slf4j.org/apidocs/org/slf4j/Logger.html)。这里有两种方法访问它，这里选择使用[依赖注入](terra-api.api-concepts.dependency-injection.md)方法实现：
...
恭喜！你成功创造了第一个附属！若要在示例拓展中实现额外的、更高级的功能，请继续阅读介绍教程。如果你打算自行研究 API，那么请阅读“[API 概念](terra-api.api-concepts.md)”部分。