...
先[创建一个空白的配置文件](config-packs.config-development.config-files.md#创建配置文件)，名称为 `ocean_biome.yml`
...
不要忘了为源[流水线群系](config-packs.config-documentation.config-objects.pipelinebiome.md)替换 `CONSTANT` 采样器，不然世界上就只会生成 `OCEAN_BIOME` 了。

加入新的 `OCEAN_BIOME` 之后，即可看到干涸的海洋生成在了世界中。

![](images/oceans-empty.png)

### 3. 添加海洋调色板

海洋中必须充满水，这可以通过添加海洋调色板实现。

通过你[选择的编辑器](config-packs.config-development.config-development-introduction.md#选择编辑器)打开 `OCEAN_BIOME`。
...
![](images/oceans-issue.png)

### 4. 添加抽象配置

为了让海洋群系的配置更加简便，我们需要用到抽象配置。

抽象配置可以避免在多个文件中重复配置[参数](config-packs.config-development.the-config-system.md#参数)，对于在群系间通用的几个[参数](config-packs.config-development.the-config-system.md#参数)非常有用。

[创建一个空白配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `base.yml`。
...
`abstract` [参数](config-packs.config-development.the-config-system.md#参数)设置为 `true` 后，`BASE` 不需要配置文件类型为 `BIOME` 时的一些[参数](config-packs.config-development.the-config-system.md#参数)。

在这个 `BASE` 配置里的所有[参数](config-packs.config-development.the-config-system.md#参数)可以应用至任意 `BIOME` 配置。

### 5. 导入抽象配置

为了让群系配置继承[参数](config-packs.config-development.the-config-system.md#参数)，首先我们需要引入 `BASE` 配置。
...
因为 `BASE` 被列在了 `OCEAN_BIOME` 配置中 `extends` 的[参数](config-packs.config-development.the-config-system.md#参数)部分，`OCEAN_BIOME` 将会继承 `BASE` 中的[参数](config-packs.config-development.the-config-system.md#参数)。
