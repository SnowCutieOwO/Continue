...
如果你还没有准备好，请在开始阅读前浏览“[配置开发介绍](config-packs.config-development.config-development-introduction.md)”与“[从零编写地形包](config-packs.config-development.creating-a-pack-from-scratch.creating-a-pack-from-scratch.md)”章节。
...
[创建一个空白配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `abstract_ore.yml`。

通过 `type` [参数](config-packs.config-development.the-config-system.md#参数)设置[配置类型](config-packs.config-development.the-config-system.md#配置类型)，并按如下示例配置 `id`：
...
[创建一个空白配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `coal_ore.yml`。
...
[创建一个空白配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `coal_ore_feature.yml`。
...
将高亮部分的[分布器](config-packs.config-documentation.config-objects.distributor.md)、[定位器](config-packs.config-documentation.config-objects.locator.md)与结构设置加入配置。
...
[分布器](config-packs.config-documentation.config-objects.distributor.md)上限是一个用于表示每区块矿物量的平均值，会除以 256。

[定位器](config-packs.config-documentation.config-objects.locator.md) 通过带标准差的 `GAUSSIAN_RANDOM`，添加了最小与最大范围，会除以 6 以拟合 3 个范围内的标准差（约 99.7% 的结果）。因此，矿物生成密度在中间值附近趋于更高。

`structures.structures` 设置为使用先前创建的 `COAL_ORE` [矿物](config-packs.config-documentation.config-files.ore.md)配置。
...
通过“[设置新地物](config-packs.config-development.creating-a-pack-from-scratch.creating-a-feature-from-scratch.md)”章节引入的 `generation-stage-feature` 附属，我们可以为矿物添加一个新生成阶段。
...
[创建一个空白配置文件](config-packs.config-development.config-files.md#创建配置文件)，名为 `ores_default.yml`。
