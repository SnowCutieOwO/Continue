...
如果排行榜上没有显示玩家（只显示了 `---`），这可能是这两个常见问题的结合：
...
这条消息意味着 ajLeaderboards 找不到你说的排行榜。
...
关于如何从 HolographicDisplays 迁移至 DecentHolograms 的教程详见[这里](../DecentHolograms/general/compatibility.md#holographicdisplays)。
