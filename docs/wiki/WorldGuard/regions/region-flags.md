...
::: warning
上述的这些标志都是指定玩家的。例如，如果将 `block-break` 标志设置为 deny，这会导致**活塞无法推或拉动方块**。

原因非常简单。玩家可以从外界向领地内投掷点燃的 TNT，或者制作一个活塞虫进入其他人的领地。这些操作是玩家有意为之，而因此找到建造者非常困难。无论如何，你还是需要保护主城，以防某些人用 TNT 大炮破坏地形。

其中一种错误的解决办法是直接阻止 TNT 大炮或者活塞。这两个东西在某些情况下仍有用处。例如，TNT 大炮和活塞应该能在区域*之内*使用。

首先，成员是可以在区域内进行建造行为的：它**不是**玩家，而是**成员**。活塞或 TNT 应当与玩家视作同一类型。那么 WorldGuard 如何判断一个活塞机器或者 TNT 大炮是领地成员？**如果它在区域内**，那它就是！

当你创建一个区域并没有设置任何标志时：
* 成员可以建筑；
* 非成员**不可以**建筑。
在区域内部的 TNT 大炮和活塞能正常工作，因为它们是该区域的“成员”。一个玩家，例如“张三”，不是这个区域的成员，无法放置或破坏方块。当你把他添加为区域成员之后，他就可以建筑了。

设置保护标志时会覆盖此类行为。如果你将 `block-break` 设置为了 `deny`，那么即使是区域成员也无法破坏方块。同样，上述的张三也不能破坏方块。区域内的 TNT 也不能破坏方块。区域内的活塞也不能破坏方块。**但是你可以破坏活塞。**
这会带来两个问题：
...
::: warning
治疗和饱食度相关标志需要配置文件中“使用玩家移动事件” 的设置**保持**开启。
:::
