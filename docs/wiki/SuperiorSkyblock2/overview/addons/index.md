...
在使用 OneBlock 附属是，你仍然需要配置 SuperiorSkyBlock。这表示你需要修改原有的空岛结构（你也不想玩单方块空岛的时候用了普通空岛的结构吧？），同时需要按需修改你的配置文件。结构的中心方块为生成方块（即单方块），因此创建[新结构](../schematics.md)时要注意这点。

## AcidIslands 附属

AcidIsland 是一种大幅修改了原版空岛体验的附属。空岛不再像普通结构那样生成，而是被洋洋大海中的一座小岛替代。里面的水有剧毒，千万不要触摸！

你可以在 [Github 介绍页](https://github.com/OmerBenGera/SSB-AcidIslands)上找到附属的源代码及发行文件。

## ProxyBridge 附属

ProxyBridge 允许你在多个子服之间同步 SuperiorSkyblock2 的数据。这个模块负责处理子服间的数据同步，是接近开服前沿的大型服务器的最佳选择。

你可以在 [Patreon](http://bg-software.com/patreon/) 上购买这个附属。
