...
@[video](./_videos/action-bar-example.mp4)
...
@[video](./_videos/title-example.mp4)
...
有效颜色为 `PINK`、`BLUE`、`RED`、`GREEN`、`YELLOW`、`PURPLE` 以及 `WHITE`——无效颜色会重置为粉色。

有效样式为 `PROGRESS`、`NOTCHED_6`、`NOTCHED_10`、`NOTCHED_12` 以及 `NOTCHED_20`。

::: info

覆盖选项只在支持 MiniMessage 的服务器上生效（见下）——在其他服务器上，BossBar 只会显示为无分隔的样子。
...
@[video](./_videos/interactable-messages-example.mp4)
...
如果你的服务器支持 MiniMessage（Paper 及其下游分支，大于 1.18 的版本），你可以在语言文件中使用 MiniMessage 格式，插件会自动转化并正确解析。

::: warning

你不能同时在一条消息里使用 MiniMessage 格式与旧版颜色代码！
