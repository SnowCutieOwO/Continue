...
如下的部分权限并非硬编码，而是通过 `interactables.yml` 与 `entity-categories.yml` 文件注册的方块交互权限（交互、使用、打开箱子、告示牌交互、耕地踩踏、海龟蛋踩踏、取出讲台书本）与实体相关权限（动物生成、伤害动物、怪物生成、伤害怪物、悦灵交互、铜傀儡交互、马匹交互、鹦鹉螺交互、装起美西螈、装起鱼、物品展示框、挂画、伤害驯服动物、矿车放置、破坏矿车）。你可以在文件中添加新标志名称，为其他类别的实体注册新标志，而无需手动编写代码。
...
* **交互**  
  允许与岛屿内的方块交互。  
  交互方块可在 `interactables.yml` 文件下自行配置。
* **邀请成员**  
  允许将其他玩家邀请至岛屿。
* **岛屿仓库**  
  允许使用岛屿上的仓库。
