...
升级是插件升级模块的一部分。因此，它们需要在位于 `plugins/SuperiorSkyblock2/modules/upgrades/config.yml` 的模块设置而非主配置文件 `config.yml` 中配置。

::: info

若你有旧版本插件的 `upgrades.yml` 文件，它会自动迁移至模块的配置文件中。

:::

除了升级本身，模块的配置文件还包含了这些全局设置：

|字段|默认值|描述|
|---|---|---|
|`enabled`|`true`|是否启用模块。关闭后，所有模块命令及功能都将禁用。|
|`crop-growth`|`true`|是否启用作物生长调整。关闭后，插件不会修改作物生长速度。|
|`mob-drops`|`true`|是否启用怪物掉落倍率调整。关闭后，插件不会修改怪物掉落倍率。|
|`island-effects`|`true`|是否启用岛屿效果。关闭后，插件不会给岛屿内的玩家任何状态效果。|
|`spawner-rates`|`true`|是否启用刷怪笼速率调整。关闭后，插件不会修改刷怪笼速率。|
|`block-limits`|`true`|是否启用方块数量限制。关闭后，插件不会限制玩家放置的方块数量。|
|`entity-limits`|`true`|是否启用实体限制。关闭后，插件不会限制岛屿上的实体生成数量。|
...
|字段|类型|描述|
|---|---|---|
|`price-type`|字符串|处理的价格类型。可选参数；省略时视作 `money`。大小写敏感。若使用无效价格类型，则跳过对应等级。|
|`price`|双精度浮点数|升级所需价格。|
|`commands`|列表|购买当前等级时一并执行的命令。可用内建变量 `%player%` 返回玩家名称。|
|`permission`|字符串|升级到此等级所需的权限。|
|`required-checks`|列表|购买当前等级所需的额外条件。详情见下。|

### 所需检查

利用 `required-checks` 参数，你可以设置玩家购买等级时需要满足的额外条件。列表中的每个条目头需要按照 `<条件>;<失败提示>` 的格式填写：条件由 JavaScript 引擎判断（支持变量），若未满足，则将失败提示发送给玩家。
...
|字段|类型|描述|
|---|---|---|
|`crop-growth`|双精度浮点数|当前升级的作物生长倍率。|
|`spawner-rate`|双精度浮点数|当前升级的刷怪笼速率。|
|`mob-drops`|双精度浮点数|当前升级的掉落倍率。|
|`team-limit`|整数|当前升级的队伍人数限制。|
|`warps-limit`|整数|当前升级的地标数量限制。|
|`coop-limit`|整数|当前升级的合作人数限制。|
|`border-size`|整数|当前升级的边界大小。不可超过主配置的 `max-island-size` 大小，否则跳过此等级。|
|`bank-limit`|字符串|当前升级的银行存款上限。支持大数字。|
|`block-limits`|配置部分|当前升级的方块数量限制。填入内容需保持 `类型: 限制` 的格式。方块类型支持数据值，格式为 `类型:值`。|
|`entity-limits'`|配置部分|当前升级的实体数量限制。填入内容需保持 `类型: 限制` 的格式。|
|`generator-rates`|配置部分|当前升级的刷石机概率。每个世界环境都有不同的概率配置（`notmal`、`nether` 或 `the_end`），填入内容均需保持 `类型: 概率` 的格式。直接填入配置部分下的概率（旧格式）则会应用至插件的默认世界。|
|`island-effects`|配置部分|当前升级的岛屿效果。填入内容需保持 `效果: 等级` 的格式，`等级` 为游戏内的效果等级（`SPEED: 1` 则给予速度 I）。忽略无效的效果名称。|
|`role-limits`|配置部分|当前升级的职位限制。填入内容需保持 `职位 ID: 限制` 的格式，`职位 ID` 为主配置中职位的数字 ID（权重）而非其名称。|
