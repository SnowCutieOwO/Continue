...
地标分类菜单是分页菜单，它会显示一列可用的物品——在这里就是地标分类。它的数量可能会大于菜单实际拥有的格子数量，这种情况下菜单就需要分成多页。
...
地标分类有一个可以配置的额外部分——`edit-lore`。当玩家有权限编辑某个地标时，这些内容就会追加在对应地标图标的描述之后。
