...
部分菜单包含了本章节未提及的额外设置（如[结构](../schematics.md)菜单）

:::

::: info

插件将基础文件与不同版本后缀的配置一并保存（例如 `warps1_12.yml`、`warps1_16.yml`）。这些版本只有材料名及用于旧版本 Minecraft 的数据值有所不同——插件会根据你的服务器版本自动选择最接近的版本。你只应该编辑与服务器版本相近的那一版。
...
|`skip-one-item`|若只有一个物品，是否直接展示物品而非打开菜单。|分页菜单|
|`custom-order`|用于决定分页菜单内容自定义排序的槽位数字列表。|分页菜单|
...
|`bannerMeta`|旗帜图案部分。每部分都是不同[颜色](https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/DyeColor.html)的[图案](https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/block/banner/PatternType.html)。|
|`trim`|盔甲纹饰部分。必须包含 `material` 参数（纹饰质地）及 `trim` 参数（纹饰样式）。仅支持 1.20+|
|`itemModel`|设置物品的物品模型。用来给物品添加自定义资源包的贴图。仅支持 1.21.4+|
|`rarity`|设置物品的稀有度，影响物品的默认颜色。仅支持 1.20.6+|
|`source`|指向物品配置的其他部分。|
|`hideTooltip`|隐藏指向物品时显示的提示框。|
...
部分菜单（群系、岛屿创建、权限管理以及设置界面等）都用到了多状态按钮，它们显示的内容取决于玩家是否可以使用它们。与一般的单个物品不同，这些按钮包含诸如 `access` 及 `no-access` 的子配置（或根据菜单变化的名称，如 `settings-enabled` 与 `settings-disabled`），每个子配置都是独立的物品配置。浏览指定菜单章节了解不同状态可填入的名称。
...
|`require-island`|打开菜单的玩家是否已经创建岛屿。|可选|
