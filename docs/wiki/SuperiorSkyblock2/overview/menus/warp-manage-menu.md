...
地标管理菜单用于管理指定地标，有一些必须配置的图标。只需在排版界面中为其分配自定义单字符即可。

|字段|类型|描述|
|---|---|---|
| `warp-rename` | 单字符 | 重命名分类的按钮。 |
| `warp-icon` | 单字符 | 打开[图标编辑菜单](warp-icon-edit-menu.md)的按钮。 |
| `warp-location` | 单字符 | 将地标位置移动到玩家脚下的按钮。 |
| `warp-private` | 单字符 | 切换地标开放状态的按钮。 |
| `success-update-sound` | 声音 | 地标更新成功后播放的声音。 |
