...
访客菜单是分页菜单，它会显示一列可用的物品——在这里就是访客。它的数量可能会大于菜单实际拥有的格子数量，这种情况下菜单就需要分成多页。
...
访客菜单有一个可以配置的额外部分——`unique-visitors`。对应着打开[独特访客菜单](unique-visitors-menu.md)的单字符，这个菜单里会显示岛屿的所有访客。
