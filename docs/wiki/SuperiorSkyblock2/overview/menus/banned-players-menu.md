...
玩家封禁菜单是分页菜单，它会显示一列可用的物品——在这里就是玩家封禁列表。它的数量可能会大于菜单实际拥有的格子数量，这种情况下菜单就需要分成多页。
...
这是玩家封禁界面的示例配置，涵盖了本章节教程提及的大部分技术及功能。
