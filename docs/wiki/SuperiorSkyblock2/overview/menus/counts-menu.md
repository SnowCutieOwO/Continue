...
方块统计界面是分页菜单，它会显示一列可用的物品——在这里就是方块。它的数量可能会大于菜单实际拥有的格子数量，这种情况下菜单就需要分成多页。
...
对应按钮会用被统计的方块作为按钮——设置中的 `type` 参数只在方块不显示时代替显示。
...
* `slots` - 
  ...
  * `{2}` - 方块的总价值
  * `{3}` - 方块的总等级
  * `{4}` - 方块的总价值，按缩写格式显示。`K` 表示千，`M` 表示百万，以此类推。
  * `{5}` - 方块的总等级，按缩写格式显示。`K` 表示千，`M` 表示百万，以此类推。
