...
如下的四个实体生成标志（自然生成动物、自然生成怪物、刷怪笼生成动物、刷怪笼生成怪物）并非硬编码，而是通过 `entity-categories.yml` 文件的 `NATURAL_SPAWN` 及 `SPAWNER_SPAWN` 动作注册的。你可以为这些动作添加新标志名称，为其他类别的实体注册新标志，而无需手动编写代码。
