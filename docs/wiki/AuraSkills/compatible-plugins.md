...
启用[经济奖励](rewards.md#经济奖励money)与[职业](main-config/index.md#职业)功能。你需要一个支持 Vault 的经济插件，如 EssentialsX 或 CMI（等支持的经济插件）。

### WorldGuard

增加了控制玩家获取经验的自定义标志。同时用于检查指定位置玩家放置/破坏方块的行为。你可以设置无视放置检查的区域，和/或设置完全阻止经验获取的区域。

见[主配置：WorldGuard 部分](main-config/index.md#worldguard)了解更多。
...
增加[攻击伤害指示器](main-config/index.md#攻击指示悬浮字)。
