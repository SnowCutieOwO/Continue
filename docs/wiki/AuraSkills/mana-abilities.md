...
2. 在 `messages/global.yml` 文件中，通过移除 `action_bar.idle` 内的 `\{\{action_bar.mana_display\}\}` 部分与多余空格。因为空闲状态的 AcionBar 只会显示生命值，你也可以将 `config.yml` 下的 `action_bar.idle` 设置为 false 将其禁用。
