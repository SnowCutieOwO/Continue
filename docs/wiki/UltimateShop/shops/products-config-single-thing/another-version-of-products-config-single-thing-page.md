...
* 物品：参与交易的物品，玩家会在交易之后得到或失去对应物品。（如果对应类型的物品不存在，即出售物品没有在配置中），则玩家交易后不会获得或失去物品，不过 `buy-actions`、`sell-actions`、`buy-conditions`、`sell-coditions` 之类的设置会正常生效。
...
``` YAML
    products: # 或 buy-prices / sell-prices
      1:
        material: emerald
...
``` YAML
   # 因为这个物品不作为真实物品存在，我们会在这里将其设置为展示物品。
   # 否则它不会在菜单中显示。
   display-item:
      material: PAPER
      ...
      1:
        ...
        give-actions:
          1:
            ...
            command: 'flyitem give {player} {amount}' # 将自定义物品给予命令填入这里!
          2:
            type: message
            message: '测试消息'
        amount: 64
...
``` YAML
   buy-prices:
      1:
        ...
        placeholder: '{amount} 点生命值'
        amount: 5
        take-actions:
          1:
            multi-once: true
            type: console_command
            command: 'health take {player} {amount}' # 示例伪命令，服务器中不存在。
```
      ...
      1:
        ...
        give-actions:
          1:
            multi-once: true
            type: console_command
            command: 'health give {player} {amount}' # 示例伪命令，服务器中不存在。
        take-actions:
          1:
            multi-once: true
            type: console_command
...
* `products`：用于出售的物品。支持“[物品格式](../../format/itemformat/index.md)”与“[经济格式](../../format/economyformat.md)”。你也可以根据单条目内容添加[自定义出售匹配方法](../../features/custom-item-match-method.md)。**可选。若不设置，玩家在交易后不会获得任何东西。适用于命令商店。**
    * `products.apply-conditions`：只有满足条件的玩家才可应用这项内容。在此使用“[条件格式](../../format/condition-format.md)”。**可选。默认无条件。**
    * `products.require-conditions`：玩家必须达到指定条件才可交易此物品。在此使用“[条件格式](../../format/condition-format.md)”。**可选。默认无条件。**
    * `products.give-actions`：在物品给予玩家后执行的动作。在此使用“[动作格式](../../format/action-format.md)”。
...
* `buy-prices`：物品的买价。支持[物品格式](../../format/itemformat/index.md)与[经济格式](../../format/economyformat.md)。你也可以根据单条目内容添加[自定义出售匹配方法](../../features/custom-item-match-method.md)。**可选。若不设置则表示物品不可购买。**
    ...
    * `buy-prices.placeholder`：显示在 `{price}` 中的内容。**必填项。**
    * `products.apply-conditions`：只有满足条件的玩家才能用这个价格交易。在此使用“[条件格式](../../format/condition-format.md)”**可选。默认无条件。**
    * `products.require-conditions`：玩家必须达到指定条件才可交易此物品。在此使用“[条件格式](../../format/condition-format.md)”**可选。默认无条件。**
...
请注意，如果你需要使用 PlaceholderAPI 或其变量拓展的变量，你需要按照我们的新格式进行配置，如下所示：
...
另外，如果菜单中有其他商品，你需要将 `menu.menu-update.click-update` 设置为 `true`，否则它们不会在你售出物品后更新。

## 单条目下的动作与条件

你可能注意到，你可以设置在单条目给予玩家时触发的动作，以及设置玩家使用该单条目的条件。这对于你播放声音或在玩家交易后执行命令时非常有用。

* 动作：在单条目配置中添加 `give-actions`/`take-actions` 部分即可。对于命令商店、权限商店、附魔商店等很有用。另外，**如果你的经济/物品插件不支持 UltimateShop，只需将其给予货币/物品的命令填入这里即可！**（`{player}` 表示玩家名称，`{amount}` 表示价格/物品数量）如果你想要让物品只执行动作，不要忘了在单条目配置中加入 `give-item: false` 设置！
...
* `give-actions` 的 `{amount}` 变量会返回单价格/物品数量，而 `buy-actions`/`sell-actions` 则会返回玩家此次购买或出售的物品数量。例如，玩家在增量购买菜单中购买了五份如下物品：
...
* `give-action`/`take-actions` 的 {amount} 返回的是 20。（玩家购买后获得了 20 个宝箱钥匙）
* `buy-action`/`sell-actions` 的 {amount} 返回的是 5。（玩家购买了五份物品）
...
``` YAML
  A:
    ...
    buy-actions: # 在商品配置中
      ...
      2:
        multi-once: true # 如果需要在命令中用到 {amount} 变量，请确保添加这行设置，保证命令在玩家大量购买时也只执行一次。
        type: console_command
...
在上述的两个示例中，最终执行效果是相同的。但你能想到吗？如果将其与 `conditions` 选项结合，通过**给予动作**方法可以为满足不同条件的玩家执行不同的命令！
