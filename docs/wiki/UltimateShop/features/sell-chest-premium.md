...
**出售魔箱**功能目前并非 UltimateShop 的主要维护项，仍处于早期开发阶段。它可能在未来版本成为正式功能，但如果包含严重问题，也可能会被移除。\
出售魔箱现可在 **4.2.0b 之后的版本**抢先体验。

:::

::: danger

**4.2.6** 之前放置的出售魔箱会在更新后失效，这是存储箱子位置的逻辑变动导致的。可以让玩家重新放置箱子解决这个问题。
...
* 不支持 Folia 服务器。
* 出售魔箱无法和其他的箱子组合成双箱。
* 出售魔箱的数据与位置依赖于当前世界的存档，若存档遭到破坏，出售魔箱的数据也不可恢复。
...
sell-chest:
    ...
    hologram:
      enabled: true
      # 可填入: DecentHolograms, CMI
      plugin: 'DecentHolograms'
```

* enabled：是否启用此功能。如果你不需要它且追求性能优化，请关闭。
* debug：启用后会在控制台显示出售魔箱相关的调试信息。

### period-ticks 和 batch-count
...
* hologram: 悬空字文本设置相关。
  * enabled：是否在出售魔箱上显示悬浮字。
  * plugin：选择使用的悬浮字插件。支持插件列表请[见此](../info/compatibility.md#直接支持的悬浮字插件列表)。
