...
如果你正在使用 Paper 1.17.1+ 及其下游分支服务端：插件会自动将物品按玩家选择的客户端语言进行翻译，比这个功能更灵活，无需**启用**。（3.8.0+）
...
* 删除生成在 `plugins/UltimateShop` 的本地化文件，使其重新生成。
* 你使用的是否为付费版本？只有付费版本才可以使用此功能。
* 插件是否在生成文件时报错？如果是的话，尝试重启服务器，插件会自动重生成文件。
* 如果你的服务器不能正常下载 Minecraft 本地化文件，那么你可以手动将通过其他渠道获取的本地化文件放在 `plugins/UltimateShop` 目录下，插件在检测到文件后会跳过下载。
* 如果服务器卡在了插件显示“下载中”部分，可以手动将语言 JSON 文件拖入服务器的 `plugins/UltimateShop` 文件夹。
