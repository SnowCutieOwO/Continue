...
在[创建商店](index.md)（管理员或玩家商店）后，你可以通过多种方式来自定义你的商店，或者将其删除。本章节描述了编辑商店的大致方法，并介绍了大部分商店都可使用的操作。
...
![](_images/admin-shop-editor-empty.png)
...
* 尚无 [Citizens NPC](citizens-shopkeepers.md) 商店的装备编辑选项。
* 也可以使用[变量物品](placeholder-items.md)来为实体装备物品栏中没有的物品。
...
* 末影人：你可以将方块放置在主手栏来修改末影人手中拿着的方块类型。
    ...
    * 你也可以使用[变量物品](placeholder-items.md)来指定方块类型。这也允许你使用那些不存在物品形式的方块（如花盆）。
...
每种商店类型的交易方式略有不同。更多信息可以在[设置管理员商店](admin-shop-setup.md)和[设置玩家商店](setup-player-shop.md)章节详细了解。
