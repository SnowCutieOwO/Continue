...
你可以在[配置文件](configuration.md)中通过修改 `language` 项切换到其他语言。例如，设置为 `langauge: de` 可以使用内置的德语。
...
请确保你的文件编码为 **UTF-8**。诸如 [Notepad++](https://notepad-plus-plus.org/) 的编辑器允许你快捷设置或修改语言文件的编码。建议在着手翻译或制作自定义语言文件时就使用 UTF-8 编码，因为文件转码可能会让某些特殊字符丢失。