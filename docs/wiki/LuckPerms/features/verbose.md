...
想要允许玩家使用某个命令，但是不知道它的权限节点是什么？

也许文档已经过时了，或者没有记录权限节点。又或许你想要调试与玩家权限相关的问题，或者只是好奇插件检查什么权限。
...
|`upload`|与“off”相同，还会将上传结果至网页浏览器以便进一步分析，上传完毕后会显示一条可供访问的链接。|

### `/lp verbose command <名称> <命令>` 的功能：

`/lp verbose command` 命令允许你在游戏内静默检查指定命令的权限。只需输入命令 `/lp verbose command <名称> <命令>`，之后 LuckPerms 便会按如下步骤快速检查一次。
...
与此类似，在浏览器使用检查系统时，你也可以设置筛选，下文会详细解释。

### 筛选

筛选是一串用于匹配结果内容的表达式，它可以隐藏你不需要的条目。它可以只是一个玩家名称，或更复杂的内容。
...
浏览器中的检查系统与游戏内大致相似，但处理大量检查时更易于分析。

![](_images/verbose-1.png)

你可以直接点击条目浏览这次检查的详情：

![](_images/verbose-2.png)