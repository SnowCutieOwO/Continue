...
本页面介绍了一些 LuckPerms API（API 本身的介绍[在此](api-introduction.md)）的用法。

除此之外，我们还有 [api-cookbook](https://github.com/LuckPerms/api-cookbook)。此为使用了 API 的示例 Bukkit 插件，用以执行一些基本操作。
...
这个方法会返回一个 `CompletableFuture`（详见[此处](api-introduction.md#使用-completablefutures)）
...
不要忘了[保存](api-usage.md#保存变动)你的改动！
