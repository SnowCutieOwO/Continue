...
HuskSync 支持同步许多数据，所有同步的数据种类均可按照你的喜好更高。请浏览“[同步功能](../../features/sync-features/index.md)”章节以获取可同步数据的完整列表。
...
检查[兼容性](../compatbility.md)中的表格。除了最新版的 Minecraft 之外，最新版的 HuskSync 也会基于热门程度和模组情况支持一些稍旧的指定版本。
...
你需要一个[数据库](../config/database.md)服务器，一个 [Redis](../config/redis.md) 服务器，以及[版本合适的 Minecraft](../compatbility.md)。
:::

::: details 是否需要 Redis？什么是 Redis？

是的，HuskSync 需要一个 [Redis] 服务器**以及[数据库](../config/database.md)服务器**才可使用。
...
更多信息请浏览 [Redis 安装教程](../config/redis.md)。
...
是的，HuskSync 需要一个[数据库](../config/database.md)服务器和 [Redis](../config/redis.md) 服务器才可使用。
