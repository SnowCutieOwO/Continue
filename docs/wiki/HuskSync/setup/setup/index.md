...
本章节将会引导你安装 HuskSync 至你的 Spigot 或 Fabric 群组子服上。请先检查服务器[兼容性](../compatbility.md)，并为服务器下载对应版本的 HuskSync。
...
* 运行了 MySQL v8.0+、MariaDB v5.0+、PostgreSQL 或 MongoDB 之一的[数据库](../config/database.md)服务器。
* [Redis](../config/redis.md) 数据库（5.0 或更高版本）
...
* 开启，之后关闭服务器，使 HuskSync 生成一份[配置文件](../config/index.md)；
...
* 若你需要从 HuskSync 1.x 版本或 MySQLPlayerDataBridge 导入数据，请浏览下方的教程：
  * [从旧版迁移](../setup/legacy-migration.md)
  * [从 MPDB 迁移](../setup/mpdb-migration.md)
