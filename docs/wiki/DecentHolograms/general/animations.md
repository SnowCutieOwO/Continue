...
自定义动画可以在文件中进行配置，更多内容会在[这里](configuration/animation.md)讲到。
...
::: info 提示
若要在动画中使用变量，你应该在 [`config.yml`](configuration/config.md) 下启用 `allow-placeholder-inside-animation` 设置，
:::
...
示例动画文件会在插件初次安装时生成，你也可以在[这个章节](configuration/animation.md)找到它。