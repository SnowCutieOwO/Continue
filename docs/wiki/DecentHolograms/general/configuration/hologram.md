...
在你可以手动编辑文档时，推荐[使用命令](../commands.md)，这样可以防止配置格式不正确导致的错误。
...
* **类型**：字符串
* 悬浮字的内容。其中的特殊行类型及其格式请见“[格式与颜色](../formats-colors/index.md)”章节。
...
* **类型**：配置部分
* 包含单独的[点击类型](../actions.md#点击类型)，可触发不同的操作。

##### `pages[*].actions.RIGHT`

* **类型**：字符串列表
* 右键点击悬浮字时触发的[动作](../actions.md)。

##### `pages[*].actions.SHIFT_RIGHT`

* **类型**：字符串列表
* Shift + 右键点击悬浮字时触发的[动作](../actions.md)。

##### `pages[*].actions.LEFT`

* **类型**：字符串列表
* 左键点击悬浮字时触发的[动作](../actions.md)。

##### `pages[*].actions.SHIFT_LEFT`

* **类型**：字符串列表
* Shift + 左键点击悬浮字时触发的[动作](../actions.md)。