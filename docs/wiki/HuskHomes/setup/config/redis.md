...
HuskHomes 支持 Redis，并可以此作主/副通信频道处理跨服传送及传送请求，以及其他[命令](../../features/commands/index.md)

## 配置

若要配置 Redis，找到你的 [`config.yml`](index.md)，并修改 `redis` 部分下的内容。将 `broker_type` 设置为 `REDIS`。
