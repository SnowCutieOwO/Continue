...
HuskHomes 提供了一系列你可使用的命令。本章节将会为列出所有命令及其对应权限，让你更好[控制玩家的访问内容](../managing-access/index.md)。

## 命令与权限表

::: info 提示

若你安装了多个命令相似的插件，你可能需要[创建命令别称](../commands/command-conflicts.md#commandsyml-下的-huskhomes-命令别称)使 HuskHomes 能正常执行命令。
...
‡ — 需要在 [`config.yml`](../../setup/config/index.md) 中将 `return_by_death` 项启用后才可使用。
...
|绕过[传送冷却检查](command-cooldowns.md)|`huskhomes.bypass_cooldowns`|未设置|
