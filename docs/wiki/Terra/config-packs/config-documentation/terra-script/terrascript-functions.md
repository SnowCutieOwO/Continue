...
structure 函数会在当前结构中生成另一个[结构](../config-objects/structure.md)，带原点偏移及旋转。
...
sampler 函数允许配置包缓存的[噪声采样器](../config-objects/noisesampler.md)在 TerraScript 中使用，并输出噪声值。
...
返回类型：`num` - [噪声采样器](../config-objects/noisesampler.md)的输出值。
