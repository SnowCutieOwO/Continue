...
[噪声](../../config-development/noise/index.md)
...
噪声采样器生成的噪声决定了每个方块或列的[部分行为](../../config-development/noise/how-noise-distributes-things.md)。
...
<badge type="info" text="color-sampler" /> [颜色采样器](colorsampler.md) - 提取通道值的颜色采样器。

<badge type="tip" text="normalize" /> [布尔值](boolean.md) - 是否将通道约束在 [-1, 1] 的范围中。

默认值：`true`

<badge type="tip" text="premultiply" /> [布尔值](boolean.md) - 是否将通道与 alpha 通道值相乘。
...
<badge type="info" text="image" /> [图像](image.md)

<badge type="tip" text="channel" /> 通道

默认值：`GRAYSCALE`（灰度）

<badge type="tip" text="clamp-to-max-edge" /> [布尔值](boolean.md)
...
<badge type="tip" text="invert-threshold" /> [布尔值](boolean.md)

默认值：`false`

<badge type="tip" text="normalization" /> [布尔值](boolean.md)
...
<badge type="tip" text="threshold" /> [整数](intenger.md)
...
<badge type="tip" text="distance-function" /> [字符串](string.md) - 采样点位与指定位置之间的距离测算方法。
...
<badge type="tip" text="normalize" /> [布尔值](boolean.md) - 若设置为 true，则返回距离将会约束在 [-1, 1] 的范围中，否则返回原本的距离。

默认值：`false`

`-1` 表示距离为 `0`，`1` 则对应 `radius` 设置的值。任意大于 `radius` 的值都会返回 `1`。

<badge type="tip" text="point.x" /> [浮点数](float.md)

默认值：`0`

<badge type="tip" text="point.y" /> [浮点数](float.md)

默认值：`0`

<badge type="tip" text="point.z" /> [浮点数](float.md)

默认值：`0`

只在采样器以空间为基础时有效。

<badge type="tip" text="radius" /> [浮点数](float.md)
...
<badge type="tip" text="frequency" /> [浮点数](float.md)- 决定噪声的[频率](../../config-development/noise/configuring-noise-samplers.md#频率)。

默认值：`0.02`

<badge type="tip" text="salt" /> [整数](intenger.md) - 决定采样器的[种子](../../config-development/noise/configuring-noise-samplers.md#盐值)。
...
<badge type="tip" text="deviation" /> [浮点数](float.md)
...
<badge type="tip" text="frequency_0" /> [浮点数](float.md)

默认值：`0.0625`

<badge type="tip" text="impulses" /> [浮点数](float.md)

默认值：`64.0`

<badge type="tip" text="isotropic" /> [布尔值](boolean.md)

默认值：`true`

<badge type="tip" text="salt" /> [整数](intenger.md) - 决定采样器的[盐值](../../config-development/noise/how-noise-samplers-work.md#盐值)。
...
<badge type="tip" text="distance" /> [字符串](string.md) - 计算细胞中心距离的方法。
...
<badge type="tip" text="jitter" /> [浮点数](float.md) 决定细胞边缘与细胞中心的最大距离。

默认值：`1`

若设置为 `0`，则表示细胞边缘与其中心重合，得到的图像表现为标准网格状花纹。推荐填入 `-1` 到 `1` 之间的值，超出这些范围的值可能带来意料外的效果。

<badge type="tip" text="lookup" /> [噪声采样器](noisesampler.md) - 当 `distance` 设置为 `NoiseLookup` 时使用的采样器。
...
|---|---|---|
|![](_images/pacman_ghosts.png)|![](_images/pacman_ghosts_grayscale.png)|![](_images/pacman_ghosts_alpha_channel.png)|
|红通道|绿通道|蓝通道|
|![](_images/pacman_ghosts_red_channel.png)|![](_images/pacman_ghosts_green_channel.png)|![](_images/pacman_ghosts_blue_channel.png)|

\* Alpha 通道因原图不存在透明部分而表现为全白。

<badge type="info" text="frequency" /> [浮点数](float.md) 图片的频率。决定图片压缩的方式。
...
|---|---|---|
|![](_images/grayscale_circles.png)|![](_images/image_sampler_circles_frequency_1.0_zoomed.png)|![](_images/image_sampler_circles_frequency_0.25_zoomed.png)|

频率为 `0.25` 时，`0.25 像素 = 1 格方块` 或 `1 像素 = 4 格方块`（如上网格例子所述）。

:::

<badge type="info" text="image" /> [字符串](string.md) - 相对于配置包文件夹的图片路径。（Windows 用户请以 `/` 代替 `\`）。
...
|---|---|
|![](_images/grayscale_circles.png)|![](_images/mountain_heightmap.png)|
...
<badge type="tip" text="value" /> [浮点数](float.md) - 输出的值。
...
用采样器扭曲另一个采样器。见[域扭曲](../../config-development/noise/configuring-noise-samplers.md#域扭曲)了解更多。

<badge type="info" text="sampler" /> [噪声采样器](noisesampler.md) - 被扭曲的采样器。

<badge type="info" text="warp" /> [噪声采样器](noisesampler.md) - 参与扭曲的采样器。

<badge type="tip" text="amplitude" /> [浮点数](float.md) - 应用扭曲的强度。
...
<badge type="info" text="kernel" /> [列表](list.md)<[列表](list.md)<[浮点数](float.md)>>

<badge type="info" text="sampler" /> [噪声采样器](noisesampler.md)

<badge type="tip" text="factor" /> [浮点数](float.md)

默认值：`1.0`

<badge type="tip" text="frequency" /> [浮点数](float.md)
...
<badge type="info" text="base" /> [浮点数](float.md) - 地形的基础 Y 值。

<badge type="tip" text="sampler" /> [噪声采样器](noisesampler.md) - 用作高度图的采样器。

默认值：`CONSTANT`

<badge type="tip" text="scale" /> [浮点数](float.md) - 调整高度图的比例。
...
<badge type="tip" text="gain" /> [浮点数](float.md)

默认值：`0.5`

<badge type="tip" text="lacunarity" /> [浮点数](float.md)

默认值：`2.0`

<badge type="tip" text="octaves" /> [整数](intenger.md)

默认值：`3`

<badge type="tip" text="weighted-strength" /> [浮点数](float.md)
...
<badge type="tip" text="ping-pong" /> [浮点数](float.md)
...
<badge type="info" text="max" /> [浮点数](float.md)

<badge type="info" text="min" /> [浮点数](float.md)
...
<badge type="info" text="points" /> [列表](list.md<[立体样条点位](cubicsplinepoint.md)>
...
<badge type="info" text="expression" /> [表达式](expression.md) - 使用 `in`（`input` 的缩写）中变量的表达式。

<badge type="info" text="sampler" /> [噪声采样器](noisesampler.md)

<badge type="tip" text="functions" /> [映射表](map.md)<[字符串](string.md)，[数学函数](mathfunction.md)> - 额外的命名数学函数，可用在 `expression` 中。
...
<badge type="tip" text="variables" /> [映射表](../../config-development/noise/how-noise-samplers-work.md)<[字符串](string.md)，[浮点数](float.md)> - 额外的命名常量变量，可用在 `expression` 中。
...
<badge type="info" text="mean" /> [浮点数](float.md)

<badge type="info" text="sampler" /> [噪声采样器](noisesampler.md)

<badge type="info" text="standard-deviation" /> [浮点数](float.md)

<badge type="tip" text="groups" /> [整数](intenger.md)
...
<badge type="info" text="amplitude" /> [浮点数](float.md)
...
<badge type="info" text="steps" /> [整数](intenger.md)
...
<badge type="info" text="left" /> [噪声采样器](noisesampler.md)

<badge type="info" text="right" /> [噪声采样器](noisesampler.md)
...
<badge type="tip" text="samplers" /> [映射表](map.md)<[字符串](string.md)，[维度适用采样器](dimensionapplicablesampler.md)> - 为采样器分配函数名称的映射表。
...
在 EXPRESSION 中定义的采样器必须属于 `DimensionApplicableSamplers`，具体请见“[维度适用采样器](dimensionapplicablesampler.md)”部分。
...
* Stage 中的 FRACTAL_EXPAND：
  ...
  <badge type="info" text="sampler" /> [噪声采样器](noisesampler.md) - 用于随机选择流水线群系。
...
* NoiseSampler 中的 CELLULAR：
  ...
  <badge type="tip" text="lookup" /> [噪声采样器](noisesampler.md) - 距离参数被设置为 `NoiseLookup` 时使用的采样器。

* FEATURE 中的 base：

  <badge type="info" text="structures.distribution" /> [噪声采样器](noisesampler.md)
...
* PALETTE 中的 base：
  ...
  <badge type="tip" text="sampler"/> [噪声采样器](noisesampler.md)
...
* Locator 中的 SAMPLER：
  ...
  <badge type="info" text="samplers" /> [列表](list.md) <[噪声采样器](noisesampler.md)>
...
* BIOME 中的 base：
  ...
  <badge type="tip" text="terrain.sampler-2d"/> [噪声采样器](noisesampler.md)
...
* BiomeProvider 中的 PIPELINE：
  ...
  <badge type="tip" text="blend.sampler" /> [噪声采样器](noisesampler.md) - 模糊高分辨率产生的锯齿效果。
...
* base 中的 BIOME：
  ...
  <badge type="info" text="terrain.sampler" /> [噪声采样器](noisesampler.md)
