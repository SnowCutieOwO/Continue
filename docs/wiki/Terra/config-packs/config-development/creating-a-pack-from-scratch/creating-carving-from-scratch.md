...
使用缓存采样器可以在配置开发时简化[噪声采样器](../../config-documentation/config-objects/noisesampler.md)的表达式。

缓存采样器允许[噪声采样器](../../config-documentation/config-objects/noisesampler.md)表达式和 [TerraScript 的采样器函数](../../config-documentation/terra-script/terrascript-functions.md)在包中随意使用。

缓存采样器在你使用同一个[噪声采样器](../../config-documentation/config-objects/noisesampler.md)制作多个表达式时很方便。
...
通过你[选择的编辑器](../../config-development/config-development-introduction.md#选择编辑器)打开包验证文件。
...
这些采样器因使用了[分形化](../../config-development/noise/configuring-noise-samplers.md)，与先前的有略微差异。
...
`FBM` 分形类型结合了多层（或称“多倍频程”）[噪声采样器](../../config-documentation/config-objects/noisesampler.md)，赋予地形更多细节。
...
[创建一个空白配置文件](../../config-development/config-files.md#创建配置文件)，名为 `carving_land.yml`。

通过 `type` [参数](../../config-development/the-config-system.md#参数)设置[配置类型](../../config-development/the-config-system.md#配置类型)，并按如下所示配置 `id`。
...
推荐阅读[“从零创建地形”](../../config-development/creating-a-pack-from-scratch/creating-terrain-from-scratch.md)和“[TerraScript 格式](../../config-documentation/terra-script/terrascript-syntax.md)”更好了解这些配置的格式。
...
通过你[选择的编辑器](../../config-development/config-development-introduction.md#选择编辑器)打开 `FIRST_BIOME` 和 `SECOND_BIOME` 文件。
...
如果你在载入包的同时还完成了先前一步的[海洋生成](../../config-development/creating-a-pack-from-scratch/creating-oceans-from-scratch.md)，你就会见到下图所示的浮空水，这是因为 `CARVING_LAND` 把这些固体方块给剔除了。

下一步我们就会讲到它的解决办法。

:::

![](_images/carving-issue.png)
...
[创建一个空白配置文件](../../config-development/config-files.md#创建配置文件)，名为 `contain_floating_water.yml`。
...
与“[创建海洋](../../config-development/creating-a-pack-from-scratch/creating-oceans-from-scratch.md)”中通过海洋调色板的方法类似，我们可以让群系继承并生成抽象配置中的内容。

通过你[选择的编辑器](../../config-development/config-development-introduction.md#选择编辑器)打开 `BASE` 配置文件。
