...
`EXPRESSION` [噪声采样器](../../config-documentation/config-objects/noisesampler.md)允许通过配置表达式输出采样器，同时还允许在采样器内调用其他采样器的内建或自定义函数。

通过你[选择的编辑器](../config-development-introduction.md#选择编辑器)打开你的 `FIRST_BIOME` [群系](../../config-documentation/config-files/biome.md)配置。
...
`terrain.sampler` 包含了三个[参数](../the-config-system.md#参数)，即 `type`、`dimensions` 和 `expression`。

* `type` - 决定了生成地形的[噪音采样器](../../config-documentation/config-objects/noisesampler.md)类型。
* `dimensions` - 决定了输入采样的空间轴数量（X、Y 与 Z）。
* `expression` - 由用户决定生成地形的表达式。

地形生成由[表达式](../../config-documentation/config-objects/expression.md)输出的正负影响。

正值输出表示此处生成实心方块，反之则不生成方块。

采样器[表达式](../../config-documentation/config-objects/expression.md)中，`-y + 64` 表示临时考虑了 Y 轴影响。
...
有关于 `EXPRESSION` 与其他噪声取样器的文档可以在[这里](../../config-documentation/config-objects/noisesampler.md)找到。

有关于数学表达式的相关文档可以在[这里](../../config-documentation/config-objects/expression.md)找到。

:::

::: tip

你可以在[表达式](../../config-documentation/config-objects/expression.md)中添加变量，降低配置难度。
...
[表达式](../../config-documentation/config-objects/expression.md) `-y + 64` 返回了类似超平坦的地形，这将会是我们进一步操作的基础，我们要在其上应用 `terrain.sampler-2d` [噪声](../noise/index.md)。
...
`terrain.sampler-2d` 现在需要一个[表达式](../../config-documentation/config-objects/expression.md)进一步修改 `terrain.sampler` 的表达式创建的地形。
...
`simplex` 需要包含[噪声采样器](../../config-documentation/config-objects/noisesampler.md)所要求的[参数](../the-config-system.md#参数)，即 `dimensions` 与 `frequency`。

`frequency` 的详细介绍可以在[这里](../noise/configuring-noise-samplers.md)找到。
...
![](_images/hill-terrain.png)

## 总结

本教程只大致讲述了地形表达式的皮毛。

你可以通过更多复杂的[表达式](../../config-documentation/config-objects/expression.md)实现无限可能。表达式还可以有更多功能，例如内置函数、自定义函数以及多层[噪声采样器](../../config-documentation/config-objects/noisesampler.md)，使得地形不再只是简单的陆地，而是炫酷的浮岛。

本章节的参考配置可以在 Github 的[这个地方](https://github.com/PolyhedralDev/TerraPackFromScratch/tree/master/3-adding-terrain)找到。

::: info

Noise Tool 是一款可视化预览采样配置的工具，更多细节可以在[这里](../noise/configuring-noise-samplers.md)浏览。
