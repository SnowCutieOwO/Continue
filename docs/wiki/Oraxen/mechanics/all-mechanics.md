...
这个机制允许你让某个物品像方块那样使用。这些机制有些特殊，你可以参阅[对应的教程章节](noteblock-mechanic/index.md)。

### 点击操作

这个机制允许你在玩家点击方块或家具时执行多样的事件。它自定义性较强，因此也需要参阅[对应的教程章节](all-mechanics/clickaction-mechanic.md)。
...
通过这个机制，你可以修改 OraxenBlocks 检测的物品类型。请确保使用在[方块机制章节](noteblock-mechanic/index.md)内声明的类型。
...
这个机制允许你自定义事件、条件与动作。因为它相当特殊，因此你可以前往[对应的教程章节](all-mechanics/custom-mechanics.md)进行阅读。
