...
本章节将会指导你安装 fnm。\
fnm 是一款 Node.js 版本管理器，适用于 Windows 等多个平台。

::: warning 注意

此章节的教程是针对 Windows 系统的，其他平台正在加紧制作中。
...
* 按下 Win+R，输入 cmd，并回车。
* 输入 `winget install Schniz.fnm`，并回车。
...
* 在 `bin/` 文件夹中，创建 `fnm_init.cmd` 并添加如下内容：
...
* 设置后，在对应界面内输入 `fnm use 版本号`，如果弹出 Node 版本号，则表示安装成功。\
* 再输入 `corepack enable pnpm`，即可在环境中使用 `pnpm` 进行包管理。
