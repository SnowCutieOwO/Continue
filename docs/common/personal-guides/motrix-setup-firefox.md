...
![](_images/aria2dmi-settings-general.png)
...
![](_images/motrix-download-settings.png)

* 回到 Aria2DMI 的设置界面，点击“RPC 服务器”，再点击展开栏中的“默认服务器”。

![](_images/rpc-server-setting.png)

* 将先前的端口号和密钥一一填入此处，其他选项保持默认。

![](_images/aria2dmi-settings-example.png)
...
![](_images/download-example-qq.png)

* 这里的下载路径留空则跟随 Motrix 的默认设置，若有需要可手动填写，也可在先前的浏览器设置中进行调整。
* 点击下载，即可静默拉起 Motrix，并在右下角弹出下载开始和完毕的提示。

![](_images/example-download-started.png)

![](_images/exmaple-downloading.png)

![](_images/example-download-completed.png)

## 5. 大功告成！

现在 Motrix 会接管所有下载请求，除非你点击取消。如果取消，浏览器将会使用自带的下载器进行下载。

::: details 为什么重启电脑之后功能就失效了？

这有可能是因为你没有设置 Motrix 的开机自启导致的。

如果下载器在线，Aria2DMI 的设置界面会弹出一条“服务器已连接”的提示。

按上文所述进入 Motrix 的设置界面，在基础设置 - 启动一栏找到“开机自动启动”，勾选即可。
