...
你可以在插件的 `items` 文件夹下创建自己的物品，只需新建一个 **yml** 格式的文件并按[物品格式](format.itemformat.md)填写即可。文件名即为物品 ID。
...
* 物品格式：将物品解析为[物品格式](format.itemformat.md)。
...
你可以在此使用[物品格式](format.itemformat.md)。在物品格式中，这里有一个称作 `material` 的选项。默认情况下，你需要输入原版物品 ID，但你也可以填入保存物品的 ID，让插件直接获取保存物品而非生成。
