...
所有游戏设置规则只能在 `launcherRules` 里面写，不能在其他任何地方写。
请严格按照 Json 文件格式编写，若出现错误则任何规则都不会识别。
请注意，只要是数组数据结构的一律有使用优先级；也就是谁在前谁就会优先使用。
如果某个版本有多个适用规则则只会读取第一个有效的，其余均无效。
支持使用正则表达式对游戏名称（即 `minecraft/versions` 目录下的文件夹名称）进行匹配校验。
...
`minMemory` 项的值不能低于 `1024` ，否则该项检测会自动失效。
