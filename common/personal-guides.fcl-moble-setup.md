...
请注意，直装包里个别文件中会有一个名为 `version` 的文件，该文件不能删去，不然游戏无法启动，注意，不能删去！不能删去！不能删去！
...
![](images/fcl-package7.png){: width="45%"}![](images/fcl-package8.png){: width="45%"}
...
::: info
 ...
 直装包最大魅力在于启动器各参数支持自定义，接下来将介绍 `assets` 下各修改配置项的文件。
...
### 4. config.json
[MinecraftArgs](https://zhuanlan.zhihu.com/p/12840515737) 修改可参考
##### 默认配置：
```
{
  # 自动选择下载方式（官方源/镜像源）
...
##### 解决无法使用中文的问题
由于使用中文，显示内容可能乱码，所以可以将 `中文` 转化成 `Unicode`
...
:::
现在，第二种方法为删去`assets\app_runtime` 下的部分文件
接下来，我将介绍该文件夹下各文件的作用
|文件夹名|作用|
|---|---|
...
|java|各种Java运行环境|
|jna|允许Java程序调用系统本地库（不能动）|
|lwjgl|游戏运行时需要用到的库，这是 `Pojav` 启动端（不建议动）|
|lwjgl-boat|游戏运行时需要用到的库，这是 `Boat` 启动端（不建议动）|
##### 使用说明
一般情况下，只需对`caciocavallo`或`java`资源进行修改即可，其他文件不推荐改动！！！
比如，我删除了`Java 11`和`Java 17`运行环境（由于我删除了`Java 17`和`Java 11`，所以`caciocavallo11`和`caciocavallo17`不在保留）